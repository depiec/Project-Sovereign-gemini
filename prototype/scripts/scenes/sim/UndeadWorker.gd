...
# Can be possessed and gains levels through service.
...
func _physics_process(delta):
	...
	if retarget_cooldown > 0: retarget_cooldown -= delta
	if is_busy:
		handle_task(delta)
		return
	if retarget_cooldown <= 0: find_work()
	if current_target_grid != Vector2i(-1, -1): move_to_target(delta)
...
func level_up():
	...
	speed += 0.5
	work_damage += 5.0
	print(name, " leveled up to ", level, "! Sasuga!")
...
func find_work():
	...
	if is_busy and current_type == dungeon_manager.TileType.WALL: return
...
func move_to_target(delta):
	...
	if type == dungeon_manager.TileType.WALL or type == dungeon_manager.TileType.REINFORCED_WALL:
		if dist < 1.6 or is_on_wall(): start_task()
		else:
			velocity = dir * current_speed
			move_and_slide()
			check_stuck(delta)
	else:
		if dist < 0.5: start_task()
		else:
			velocity = dir * current_speed
			move_and_slide()
			check_stuck(delta)
...
func check_stuck(delta):
	if velocity.length() < 0.5: stuck_timer += delta
